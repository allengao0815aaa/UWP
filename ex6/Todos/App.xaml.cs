...
namespace Todos
{
    ...
    sealed partial class App : Application
    {
        ...
        public App()
        {
            ...
            string sql = @"DROP TABLE IF EXISTS TodoItem;";
            ...
            using (var statement = conn.Prepare("INSERT INTO TodoItem (Title, Context, Date) VALUES (?, ?, ?)"))
            {
                statement.Bind(1, "first");
                statement.Bind(2, "Hello!");
                statement.Bind(3, DateTime.Today.ToString("yyyy/MM/dd"));
                statement.Step();
            }
            using (var statement = conn.Prepare("INSERT INTO TodoItem (Title, Context, Date) VALUES (?, ?, ?)"))
            {
                statement.Bind(1, "second");
                statement.Bind(2, "Nice to meet you!");
                statement.Bind(3, DateTime.Today.ToString("yyyy/MM/dd"));
                statement.Step();
            }
        }
